...
export default function VideoCardHent() {
  ...
  //"videos" bruges til at lagre listen over videoer, og "isVideos" bruges til at kontrollere, om der er videor at vise.
  const [videos, setVideos] = useState([]);
  const [isVideos, setIsVideos] = useState(true);

  useEffect(() => {
    async function getVideos() {
      //Der defineres en URL til at hente videornes data fra min Firebase-database.
      const url =
  ...
  return (
    <article>
      {isVideos ? (
        <div>
          {videos.map((video) => (
            <Playlist key={video.id} book={video} />
          ))}
        </div>
      ) : (
        <p>Nothing to show</p>
